...
router.get(["/api/list/hotellist/:order_uuid"], async (req, res) => {
  const [rows] = await db.query(
    "SELECT `product_name`,`hotel`.`product_number`,`orders_details_hotel`.`quantity`,`orders_details_hotel`.`total_price`,`hotel_room`.`room_price`,`orders_details_hotel`.`commented`  FROM `hotel`JOIN `orders_details_hotel` JOIN `hotel_room` JOIN `orders` ON `orders`.`uuid`=`orders_details_hotel`.`orders_uuid` AND `orders_details_hotel`.`hotel_products_sid`=`hotel`.`sid` AND `hotel_room`.`product_number`=`hotel`.`product_number` AND `orders_details_hotel`.`room_type`=`hotel_room`.`room_type` WHERE `orders`.`uuid`=?",
    [req.params.order_uuid]
  );
  res.json(rows);
  //不做分頁
